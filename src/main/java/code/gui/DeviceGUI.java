...
public class DeviceGUI extends Thread
{
	...
	public void flashBlockInfo()
	{
		...
		inputBar.setToolTipText("??Ч???????: "+time);
		inputText.setText("");
		if(last!=1)
		{
		...
		outputBar.setToolTipText("??Ч???????: "+time);
		outputText.setText("");
		if(last!=1)
		{
		...
		if(last!=3)
		{
			...
			for(int i=0;i<num;++i)
			{
				...
				printerText.append("????????: "+printerID[i]);
			}
		}
		// ?????
		...
		cameraBar.setToolTipText("??Ч???????: "+time);
		cameraText.setText("");
		if(last!=1)
		{
